...
RMenu.Add('example', 'main', RageUI.CreateMenu("", "~p~ID card"))
RMenu.Add('example', 'chargeur', RageUI.CreateSubMenu(RMenu:Get('example', 'main'), "", "~p~ULRPRP - ID card"))

Citizen.CreateThread(function()
    while true do
        RageUI.IsVisible(RMenu:Get('example', 'main'), true, true, true, function()

            RageUI.Button("ID card", "Obtain an identity card", {RightLabel = "~g~100$"}, true, function(Hovered, Active, Selected)
                if (Selected) then
                ...
                RageUI.IsVisible(RMenu:Get('example', 'knife'), true, true, true, function()
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            for k in pairs(position) do
                ...
                if dist <= 4.0 then
                    ESX.ShowHelpNotification("Press ~ INPUT_CONTEXT ~to buy your ~ p ~ identity card")
...
Citizen.CreateThread(function()
    while true do
        ...
        if Vdist2(GetEntityCoords(PlayerPedId(), false), v1) < distance then
            Draw3DText(v1.x,v1.y,v1.z, "[LARRY]=Buy Id Card")
        end
    end
end)
